...
public class LevelManager : MonoBehaviour
{
    ...
    private void InitializeLevel()
    {
        ...
        // --- defaults defensivos para JSON incompleto ---
        if (currentLevelData == null)
        {
            currentLevelData = new LevelData();
        }

        if (currentLevelData.timers == null)
            currentLevelData.timers = new Dictionary<string, float>();

        if (!currentLevelData.timers.ContainsKey("patience"))      currentLevelData.timers["patience"] = 18f;
        if (!currentLevelData.timers.ContainsKey("prep_pan"))      currentLevelData.timers["prep_pan"] = 0.3f;
        if (!currentLevelData.timers.ContainsKey("prep_sausage"))  currentLevelData.timers["prep_sausage"] = 0.5f;

        if (currentLevelData.order_mix == null || currentLevelData.order_mix.Count == 0)
            currentLevelData.order_mix = new Dictionary<string, float> { { "italiano", 1f } };
        // --- fin defaults defensivos ---
    ...
    private IEnumerator SpawnInitialCustomers()
    {
        ...
        int queueSlots = 3; // Default value
        if (currentLevelData != null && currentLevelData.queue_slots > 0)
        {
            queueSlots = currentLevelData.queue_slots;
        }
        
        for (int i = 0; i < Mathf.Min(queueSlots, 3); i++)
        {
            SpawnCustomer();
    ...
    private void ManageCustomerSpawning()
    {
        ...
        if (customerSpawnTimer >= customerSpawnInterval)
        {
            ...
            // Get queue slots with defensive check
            int queueSlots = 3; // Default value
            if (currentLevelData != null && currentLevelData.queue_slots > 0)
            {
                queueSlots = currentLevelData.queue_slots;
            ...
            if (customersInQueue < queueSlots)
            {
                SpawnCustomer();
    ...
    private void SpawnCustomer()
    {
        ...
        if (customer != null)
        {
            ...
            float patienceTime = 18f;
            if (currentLevelData != null && currentLevelData.timers != null &&
                currentLevelData.timers.TryGetValue("patience", out var p))
            {
                patienceTime = p;
            }
            
            Dictionary<string, float> orderDistribution =
                (currentLevelData != null && currentLevelData.order_mix != null && currentLevelData.order_mix.Count > 0)
                ? currentLevelData.order_mix
                : new Dictionary<string, float> { { "italiano", 1f } };
